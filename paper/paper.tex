\let\mypdfximage\pdfximage
\def\pdfximage{\immediate\mypdfximage}

\documentclass[a4paper,10pt,oneside,bibliography=totoc]{scrreprt}
\usepackage[french,english]{babel}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{textcomp}
\usepackage{graphicx}
\usepackage{mathtools}
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{siunitx}
\usepackage{fullpage}
\usepackage{wrapfig}
\usepackage{hyperref}
\usepackage{url}
\usepackage{float}
\usepackage[font={footnotesize}]{caption}
\usepackage{subcaption}
\usepackage{verbatim}
\usepackage{tabularx}
\usepackage{forloop}
\usepackage{pgffor}
\usepackage{comment}
\usepackage[toc]{glossaries}
\usepackage{placeins}
\usepackage{listings}
\usepackage{color}
\usepackage{abstract}
\usepackage{algpseudocode}

\lstset{language=C++,
	basicstyle=\ttfamily \footnotesize,
	numberstyle=\footnotesize,
	stepnumber=1,
	numbersep=5pt,
	showspaces=false,
	showstringspaces=false,
	showtabs=false, particular underscores
	frame=single,
	tabsize=2,
	captionpos=b,
	breaklines=true,
	breakatwhitespace=false,
	escapeinside={\%*}{*)}
}


%\excludecomment{figure}
%\setcounter{tocdepth}{4}
%\usepackage{pxfonts} 
%\mathtoolsset{showonlyrefs=true}

\linespread{1.2}

\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\argmin}{arg\,min}
\newcommand*{\diffd}{\mathrm{d}}
\newcommand{\matr}[1]{\mathbf{#1}}
\newcommand{\hommatr}[1]{\hat{\mathbf{#1}}}
\newcommand{\transpose}[1]{#1^\intercal}
\newcommand{\homvec}[1]{\vec{\hat{#1}}}
\newcommand{\colvecT}[1]{\left[#1\right]^\intercal}
\newcommand{\rowvec}[1]{\left[#1\right]}
\newcommand{\coreq}{\hat{=}}
\newcommand{\area}[1]{\mathcal{A}(#1)}
\newcommand{\cpp}{C\nolinebreak\hspace{-.05em}\raisebox{.4ex}{\tiny\bf +}\nolinebreak\hspace{-.10em}\raisebox{.4ex}{\tiny\bf +}}

\makeglossaries
\include{glossary}

\begin{document}

\include{title}
\include{abstract}

%\begin{comment}

\selectlanguage{english}

\tableofcontents

\include{ch1_introduction}
\glsreset{}
\include{ch2_theory}
\include{ch3_state}
\include{ch4_models}
\include{ch5_experiments}
\include{ch6_analysis_pc}
\include{ch7_method}
\include{ch8_implementation}
\include{ch9_conclusion}

\appendix
\include{chA_expriments}
\include{chB_proofs}

\printglossaries

\bibliographystyle{authordate1}
\bibliography{../reference/references}

%\end{comment}

\end{document}