...
void* mmap_allocator_base::allocate_(std::size_t length, std::size_t align, const void* hint) {
	...
	HANDLE temporary_file_handle = CreateFile(
	...
	if(ret2 == 0) {
		...
		throw std::runtime_error("mmap: SetFilePointerEx failed.");
	}

	ret2 = SetEndOfFile(temporary_file_handle);
	if(ret2 == 0) {
		CloseHandle(temporary_file_handle);
		throw std::runtime_error("mmap: SetEndOfFile failed.");
	}
	...
	if(ptr == nullptr) {
		...
		CloseHandle(temporary_file_handle);
		throw std::runtime_error("mmap: MapViewOfFile failed."); 
	} else if(std::uintptr_t(ptr) % align != 0) {
		CloseHandle(file_mapping_handle);
		CloseHandle(temporary_file_handle);
		throw std::runtime_error("Memory mapped file not properly aligned."); 
	}
