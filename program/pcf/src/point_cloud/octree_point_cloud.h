...
private:
	...
	const node& deepest_node_containing_point_(const Other_point& pt, const node& par_nd, const node_cuboid& par_cub) const;
...
public:
	explicit node(const segment& seg) : segment(seg) { }
